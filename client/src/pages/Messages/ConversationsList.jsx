...
export default function ConversationsList({ threads, activeId, onSelect }) {
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-3 font-semibold border-b">Nachrichten</div>
      {threads.length === 0 ? (
        <div className="p-3 text-sm text-gray-500">Keine Konversationen.</div>
      ) : (
        threads.map((t) => {
          const title = t.isGroup ? t.name : t.peer?.username || "Unbekannt";
